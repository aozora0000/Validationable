...
class ImageWidthRule implements RuleInterface
{
    ...
    public function passes(string $attribute, mixed $value, Parameters $parameters, array $arguments = []): bool
    {
        ...
        if (!preg_match('/\d+$/u', $arguments[0])) {
            throw new \InvalidArgumentException("Image width rule requires a valid size");
        }
        ...
        $fn = fn(Image $image): bool => match(true) {
            (bool)preg_match('/^\d+$/', $arguments[0]) => $image->width() === (int)$arguments[0],
            Str::startsWith($arguments[0], '==')=> $image->width() === $replace('=='),
            Str::startsWith($arguments[0], '<=') => $image->width() <= $replace('<='),
            Str::startsWith($arguments[0], '<') => $image->width() < $replace('<'),
            Str::startsWith($arguments[0], '>=') => $image->width() >= $replace('>='),
            Str::startsWith($arguments[0], '>') => $image->width() > $replace('>'),
