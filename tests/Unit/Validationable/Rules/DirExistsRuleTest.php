...
class DirExistsRuleTest extends TestCase
{
    #[Test]
    public function ファイルが存在する場合テスト(): void
    {
        // テスト対象のクラスを初期化
        $instance = new DirExistsRule();

        // テスト用の一時ファイルを作成
        $testFilePath = sys_get_temp_dir() . DIRECTORY_SEPARATOR . Str::random(10);
        mkdir($testFilePath);
    ...
    public function ファイルが存在しない場合テスト(): void
    {
        ...
        // 存在しないファイルパス
        $nonExistentFilePath = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'non_existent_dir';
